...
namespace Matrices
{
    public static class AugmentedMatrixOperations
    {
        ...
        public static AugmentedMatrix MultiplyRowByScalar(this AugmentedMatrix augMatrix, int row, double scalar, int startColumn = 0)
        {
            for (int i = 0; i < augMatrix.Count; i++)
            {
        ...
        /// <returns></returns>
        public static AugmentedMatrix AddRows(this AugmentedMatrix augMatrix, int augendRow, int addendRow,  double scalarMultiple = 1.0)
        {
            for (int i = 0; i < augMatrix.Count; i++)
            {
                for (int j = 0; j < augMatrix[i].Columns; j++)
                {
                    augMatrix[i][addendRow, j] = augMatrix[i][addendRow, j] + ( scalarMultiple * augMatrix[i][augendRow, j]);
                }
            }
        ...
        /// <param name="addendRow"></param>
        /// <returns></returns>
        public static AugmentedMatrix AddRows(this AugmentedMatrix augMatrix, double[] augendRow, int addendRow, int matrixIndex)
        {
        ...
        public static AugmentedMatrix GaussianElimination(this AugmentedMatrix augMatrix)
        {
            ...
            {
                augMatrix = augMatrix.MultiplyRowByScalar(
                    0,
                    1.0 / augMatrix[0][0, nonZeroColumnIndex]);
            }
